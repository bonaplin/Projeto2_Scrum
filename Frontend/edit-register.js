...
let msg = document.getElementById("error-register");
...
async function getUserInfo(username) {
  try {
    const response = await fetch(
      `http://localhost:8080/jm-rc-proj2/rest/users/${username}`,
      {
        method: "GET",
        headers: {
...
function populateFormEdit(userData) {
  ...
  document.getElementById("firstNameRegister2").value =
    userData.firstName || "";
...
async function save() {
  ...
  try {
    ...
    if (!response.ok) {
      msg.innerHTML = `Error updating information! HTTP error: ${response.status}`;
    } else {
      updateSuccess();
    }
  } catch (error) {
